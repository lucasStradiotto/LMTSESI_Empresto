(() => {
  ...
  const KEY_OCC = 'estado_celulares';
  const KEY_MAN = 'estado_celulares_manut';
  const BTN_EXPORT = document.getElementById('export-cel');
  const BTN_IMPORT = document.getElementById('import-cel');
  const FILE_IMPORT= document.getElementById('import-cel-file');
  ...
  for (let r=0;r<LINHAS;r++){
    ...
    for (let c=0;c<COLUNAS;c++){
      ...
      if (n<=TOTAL){
        ...
        const label=document.createElement('label'); label.className='notebook livre'; label.dataset.item=String(n); label.setAttribute('aria-label',`Celular ${n}`);
        const input=document.createElement('input'); input.type='checkbox'; input.id=`cel-${n}`; input.className='estado sr-only'; input.setAttribute('aria-label',`Celular ${n} — marcar como ocupado`); input.checked=!!occ[n-1];
        ...
        const img=document.createElement('img'); img.src=ICON_SRC; img.alt=`Ícone do celular ${n}`; img.title=`Celular ${n}`; img.className='notebook-icon';
        const caption=document.createElement('span'); caption.className='notebook-id'; caption.textContent=`Celular ${n}`;
        ...
        btnMaint.addEventListener('click', async (ev)=>{
          ...
          const ok=await confirmModal({
            ...
            text:  emManut?`O Celular ${n} voltará a ficar disponível.`:`O Celular ${n} ficará indisponível para empréstimo.`,
  ...
  document.querySelectorAll('.notebook').forEach(box=>{
    ...
    const input=box.querySelector('input.estado'); applyVisual(box,input,n);
  });
  ...
  function exportar(){
    ...
    for(let i=1;i<=TOTAL;i++){
      ...
      rows.push(['celular', i, `Celular ${i}`, status]);
    }
  ...
  function applyRows(rows){
    ...
    for(const cols of rows){
      if(cols.length>=4){
        const tipo=(cols[0]||'').toLowerCase(); if(!/cel/.test(tipo)) continue;
    ...
    if(updated){ save(KEY_OCC,occ); save(KEY_MAN,man); refreshDOM(); alert('Importação concluída (Celulares).'); }
    else{ alert('Nenhuma linha válida para Celulares foi encontrada.'); }
  }
